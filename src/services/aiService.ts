...
All placements MUST be within: 0<=X<=${maxX}, 0<=Y<=${maxY}, 0<=Z<=${maxZ}`,
      '',
      'AVAILABLE BLOCKS (only use these blockIds)',
      blockList,
      '',
...
- airId: ${palettes.air ?? 'none in palette'}`,
      '',
      currentState,
      'DESIGN PROTOCOL',
      `1) Plan first:
...
- Prefer perimeter-first, then fill, to reduce overlaps and keep structure clean.`,
      '',
...
blockName should match the "name" or "displayName" of the used blockId from the available list.`,
      '',
      'IMPORTANT',
      `- If the requested design cannot fit, scale it down.
